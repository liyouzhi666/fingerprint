...
{    
    ...
    ui->cboxBaudRate->setCurrentIndex(14);
...
void frmMain::ReadMyCom()
{
    ...
    if (IsShow)
    {
        if (IsHexReceive)
        {
            ...
            if(tempDataHex == "F5 05 00 00 00 00 05 F5")
                myHelper::ShowMessageBoxInfo("删除完成");
            else if(tempDataHex == "F5 03 00 00 00 00 03 F5")
                myHelper::ShowMessageBoxInfo("指纹录入完成");

            else if(tempDataHex == "F5 01 00 00 06 00 07 F5" ||
                    tempDataHex == "F5 01 00 00 07 00 06 F5")
                myHelper::ShowMessageBoxInfo("用户已存在");

            else if(tempDataHex == "F5 0C 00 01 01 00 0C F5")
                myHelper::ShowMessageBoxInfo("与用户1匹配");
            else if(tempDataHex == "F5 0C 00 02 01 00 0F F5")
                myHelper::ShowMessageBoxInfo("与用户2匹配");
            else if(tempDataHex == "F5 0C 00 00 05 00 09 F5")
                myHelper::ShowMessageBoxInfo("无此用户");
...
void frmMain::SaveMyCom()
{
    ...
    db.setDatabaseName("MyPFDB.db");
    if(!db.open()) {
        return ;
    }
    QSqlQuery query;
    query.exec("create table PFtable(InsertTime varchar, PFvalue varchar)");
    query.exec("insert into PFtable values('"+name+"','"+tempData+"')");
}
...
void frmMain::on_btnPF1_clicked()
{
    ui->txtSend->setEditText("F5 24 00 00 00 00 24 F5");
    this->WriteMyCom();
}

void frmMain::on_btnPF2_clicked()
{
    ui->txtSend->setEditText("F5 05 00 00 00 00 05 F5");
    this->WriteMyCom();
}

void frmMain::on_btnPF3_clicked()
{
    ui->txtSend->setEditText("F5 01 00 01 01 00 01 F5");
    this->WriteMyCom();
    wait();
    ui->txtSend->setEditText("F5 02 00 01 01 00 02 F5");
    this->WriteMyCom();
    wait();
    ui->txtSend->setEditText("F5 03 00 01 01 00 03 F5");
    this->WriteMyCom();

}

void frmMain::on_btnPF4_clicked()
{
    ui->txtSend->setEditText("F5 01 00 02 01 00 02 F5");
    this->WriteMyCom();
    wait();
    ui->txtSend->setEditText("F5 02 00 02 01 00 01 F5");
    this->WriteMyCom();
    wait();
    ui->txtSend->setEditText("F5 03 00 02 01 00 00 F5");
    this->WriteMyCom();
}

void frmMain::on_btnPF5_clicked()
{
    ui->txtSend->setEditText("F5 0C 00 00 00 00 0C F5");
    this->WriteMyCom();
}

void frmMain::on_btnPF6_clicked()
{
    ui->txtSend->setEditText("F5 31 00 01 00 00 30 F5");
    this->WriteMyCom();
}

void frmMain::on_btnPF7_clicked()
{
    ui->txtSend->setEditText("F5 31 00 02 00 00 33 F5");
    this->WriteMyCom();
}
